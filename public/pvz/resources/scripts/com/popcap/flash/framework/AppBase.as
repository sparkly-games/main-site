package com.popcap.flash.framework
{
   ...
   public class AppBase extends Sprite
   {
      ...
      private function handleFrame(param1:Event) : void
      {
         ...
         this.avgFlashTime = this.avgFlashTime + (_loc3_ - this.lastFlashTime);
         this.excessUpdateTime = this.excessUpdateTime + _loc4_;
         if(this.mShowDebugInfo)
         {
            this.fpsTime = this.fpsTime + _loc4_;
         ...
         this.avgUpdateTime = this.avgUpdateTime + (getTimer() - _loc5_);
         _loc5_ = getTimer();
         this.renderStep();
         this.avgRenderTime = this.avgRenderTime + (getTimer() - _loc5_);
         this.fpsCount++;
      ...
      protected function handleKeyDown(param1:KeyboardEvent) : void
      {
         ...
         if(_loc2_)
         {
            ...
            if(param1.ctrlKey)
            {
               _loc3_ = _loc3_ | CONTROL_FLAG;
            }
            if(param1.altKey)
            {
               _loc3_ = _loc3_ | ALT_FLAG;
            }
            if(param1.shiftKey)
            {
               _loc3_ = _loc3_ | SHIFT_FLAG;
            }
            _loc4_ = this.mCheatBindings[_loc3_];
            if(_loc4_ != null)
            {
               _loc5_ = this.mCheats[_loc4_];
               if(_loc5_ != null)
               {
                  _loc5_();
               }
      ...
      public function start() : void
      {
         ...
         var _loc1_:int = this.mDataXML.cheats.bind.length();
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
      ...
      public function toggleMute(param1:Boolean, param2:Boolean = false) : void
      {
         if(param2)
         {
            this.mMasterMute = param1;
         }
         param1 = param1 || this.mMasterMute;
         if(param1 == true)
         {
            this.mMuted = true;
      ...
      public function registerService(param1:BaseAppPlugin, param2:Vector.<String>, param3:Object, param4:XML) : IAppServiceRegistration
      {
         ...
         {
            _loc7_ = this.mServiceMap[_loc6_];
            if(_loc7_ == null)
            {
      ...
      private function addBinding(param1:XML) : void
      {
         ...
         var _loc3_:int = this.mCodeMap[_loc2_];
         if(_loc3_ > 0)
         {
         ...
         for each(_loc6_ in _loc4_)
         {
            ...
            if(_loc7_ == "CONTROL")
            {
               _loc5_ = _loc5_ | CONTROL_FLAG;
            }
            else if(_loc7_ == "ALT")
            {
               _loc5_ = _loc5_ | ALT_FLAG;
            }
            else if(_loc7_ == "SHIFT")
            {
               _loc5_ = _loc5_ | SHIFT_FLAG;
            }
            else
            {
               _loc3_ = this.mCodeMap[_loc7_];
               if(_loc3_ == 0)
               {
                  return;
               }
